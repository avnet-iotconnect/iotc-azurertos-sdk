...
static int g_hs300x_sensor0_quick_setup(void) {
  ...
  /* Open HS300X sensor instance, this must be done before calling any HS300X
   * API */
...
void hs300x_sensor_thread_entry(ULONG entry_input) {
  ...
  while (1) {
    ...
    case DEMO_SEQUENCE_5: {
      ...
      /* Calculate data */
      err = RM_HS300X_DataCalculate(g_hs300x_sensor0.p_ctrl, &raw_data,
      ...
      if (FSP_SUCCESS == err) {
        ...
        temperature_measurement =
            (double)(gs_hs300x_data.temperature.integer_part) +
            ((double)gs_hs300x_data.temperature.decimal_part) / 100.0;
        humidity_measurement =
            (double)(gs_hs300x_data.humidity.integer_part) +
            ((double)gs_hs300x_data.humidity.decimal_part) / 100.0;
...
double get_temperature(void) { return temperature_measurement; }

double get_humidity(void) { return humidity_measurement; }
