...
extern bool app_startup_interactive(NX_IP *ip_ptr, NX_PACKET_POOL *pool_ptr,
                                    NX_DNS *dns_ptr);
...
#define SAMPLE_POOL_SIZE                                                       \
  ((SAMPLE_PACKET_SIZE + sizeof(NX_PACKET)) * SAMPLE_PACKET_COUNT)
...
/* Define the address of SNTP Server. If not defined, use DNS module to resolve
 * the host name SAMPLE_SNTP_SERVER_NAME.  */
/*
...
void tx_application_define(void *first_unused_memory) {
  ...
  status = nx_packet_pool_create(&pool_0, "NetX Main Packet Pool",
                                 SAMPLE_PACKET_SIZE, (UCHAR *)sample_pool_stack,
  ...
  status = nx_ip_create(&ip_0, "NetX IP Instance 0", SAMPLE_IPV4_ADDRESS,
                        SAMPLE_IPV4_MASK, &pool_0, nx_driver_rx_fit,
  ...
  status = nx_arp_enable(&ip_0, (VOID *)sample_arp_cache_area,
                         sizeof(sample_arp_cache_area));
  ...
  status = tx_thread_create(
      &sample_helper_thread, "Demo Thread", sample_helper_thread_entry, 0,
  ...
  // TODO: double check usb_thread_memory_stack as it might be wrong and related
  // to sample_helper_thread_stack
  status = tx_thread_create(&usb_thread, "USB Thread", usb_thread_entry_func, 0,
                            usb_thread_memory_stack, USB_DEMO_THREAD_STACK_SIZE,
                            3, 3, TX_NO_TIME_SLICE, TX_AUTO_START);

  if (status) {
    printf("Failed to create USB thread: 0x%x\r\n", status);
    return;
  }
}
...
void sample_helper_thread_entry(ULONG parameter) {
   ...
   *  the UpdateKey family of APIs are used in combination with the
   * UpdateKeyring KeyIndex)
   */
...
static void dhcp_wait() {
  ...
  status =
      nx_ip_status_check(&ip_0, NX_IP_ADDRESS_RESOLVED, &actual_status, 1000);
  for (int i = 0; i < 10 && status != NX_SUCCESS &&
                  actual_status != NX_IP_ADDRESS_RESOLVED;
       i++) {
...
static UINT dns_create() {
  ...
  status = nx_dhcp_interface_user_option_retrieve(
      &dhcp_0, 0, NX_DHCP_OPTION_DNS_SVR, (UCHAR *)(dns_server_address),
  ...
  printf("DNS Server address: %lu.%lu.%lu.%lu\r\n",
         (dns_server_address[0] >> 24), (dns_server_address[0] >> 16 & 0xFF),
         (dns_server_address[0] >> 8 & 0xFF), (dns_server_address[0] & 0xFF));
