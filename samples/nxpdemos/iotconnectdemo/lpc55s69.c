...
static ULONG usb_memory[USB_BUFFER_SIZE / sizeof(ULONG)];
...
static UINT unix_time_get_ori(ULONG *unix_time)
{
...
static void main_thread_entry(ULONG thread_input)
{
    ...
    for (UINT i = 0; i < SAMPLE_SNTP_SYNC_MAX; i++)
    {
        ...
        status = sntp_time_sync(&nx_ip, &packet_pool, &dns_0, SAMPLE_SNTP_SERVER_NAME);
        /* Check status.  */
        if (status == NX_SUCCESS)
            break;
...
//        unix_time_base = SAMPLE_SYSTEM_TIME;
    ...
    unix_time_get_ori(&unix_time);

    srand(get_seed());

    /* Start sample.  */
    bool app_status = app_startup(&nx_ip, &packet_pool, &dns_0);
...
int main(void)
{
    ...
    PRINTF("Start the azure_iotconnect example...\r\n");
...
void tx_application_define(void *first_unused_memory)
{
    ...
    /* Initialize USBX. */
    status = ux_system_initialize(usb_memory, USB_BUFFER_SIZE, UX_NULL, 0);
    if (status != UX_SUCCESS)
        goto err;
