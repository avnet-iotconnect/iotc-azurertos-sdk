...
#ifndef IOTCONNECT_DOWNLOAD_CLIENT_RECEIVE_BUFFER_SIZE
#define IOTCONNECT_DOWNLOAD_CLIENT_RECEIVE_BUFFER_SIZE 3000
#endif

static UCHAR buffer[IOTCONNECT_DOWNLOAD_CLIENT_RECEIVE_BUFFER_SIZE];
static size_t data_length;
static size_t file_size;
static size_t file_bytes_received;

#define HDR_CONTENT_LENGHT_STR "Content-Length"
...
#define MAX_DATA_LENGTH_DIGITS 10 // 9 gigabytes.. Not that we really need this much, but to be empyrical about it...
                            ...
                            CHAR *field_value, UINT field_value_length)
{
    ...
    if (field_name_length != sizeof(HDR_CONTENT_LENGHT_STR) - 1 /* String has null in it */ ) {
        return;
    }
    if (0 != memcmp(field_name, HDR_CONTENT_LENGHT_STR, field_name_length)) {
        return;
    }
...
static UINT get_response(IotConnectHttpRequest *r, NX_WEB_HTTP_CLIENT *http_client) {
    ...
    while (get_status != NX_WEB_HTTP_GET_DONE) {
        ...
        status = nx_packet_data_extract_offset( //
                ...
                IOTCONNECT_DOWNLOAD_CLIENT_RECEIVE_BUFFER_SIZE - data_length /* 1 for null */, // bytes left in buffer
                &bytes_received);
        if (status) {
            printf("download client: Packet data extraction error: 0x%x\r\n", status);
            break;
        }
        if (bytes_received > IOTCONNECT_DOWNLOAD_CLIENT_RECEIVE_BUFFER_SIZE - data_length) {
...
static UINT request_handler(IotConnectHttpRequest *r ,NX_WEB_HTTP_CLIENT *http_client) {
    ...
    printf("download client: File size is %i\r\n", file_size);
    printf("download client: Data size %i\r\n", data_length);
    ...
    do {
        size_t size;
        if (file_bytes_received + IOTCONNECT_DOWNLOAD_CLIENT_RECEIVE_BUFFER_SIZE < file_size) {
            size = IOTCONNECT_DOWNLOAD_CLIENT_RECEIVE_BUFFER_SIZE;
        } else{
...
UINT iotc_download(IotConnectHttpRequest *req) {
    req->custom_handler_cb = request_handler;
